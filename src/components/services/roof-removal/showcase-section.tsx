        ...
        <div className="grid gap-8 lg:grid-cols-2">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <article>
                <div className="relative h-52 overflow-hidden rounded-lg border border-border/40">
                  <Image src="/images/before-roof.jpg" alt="Roof removal before" fill className="object-cover" sizes="(max-width: 640px) 100vw, 50vw" />
                </div>
                <p className="mt-2 text-xs font-semibold uppercase tracking-[0.16em] text-muted-foreground">Before</p>
              </article>
              <article>
                <div className="relative h-52 overflow-hidden rounded-lg border border-border/40">
                  <Image src="/images/after-roof.jpg" alt="Roof removal after" fill className="object-cover" sizes="(max-width: 640px) 100vw, 50vw" />
                </div>
          ...
          <Reveal className="glass-card border border-border/40 p-6 md:p-8">
            ...
            <p className="mt-3 text-sm leading-relaxed text-muted-foreground">
              We&apos;ll review your project scope, timeline, and budget, then provide a tailored plan and a clear estimate.
            </p>
            <div className="mt-6 space-y-3">
              <Button asChild className="w-full justify-center">
                <Link href="/contact">Request Detailed Estimate<ArrowRight className="ml-1 h-4 w-4" /></Link>
              </Button>
              <Button asChild variant="outline" className="w-full justify-center border-secondary/40 text-secondary hover:bg-secondary/10 hover:text-secondary">
                <a href={`tel:${siteConfig.phones.english}`}><Phone className="mr-2 h-4 w-4" />{siteConfig.phones.english}</a>
              </Button>
            </div>
          </Reveal>
        </div>
