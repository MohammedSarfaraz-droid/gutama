  ...
  return (
    <section className="relative overflow-hidden py-24 md:py-32">
      <div className="absolute inset-0">
        <Image src="/images/service-chimney.jpg" alt="Chimney services" fill priority className="object-cover" sizes="100vw" />
      ...
      <Container className="relative">
        <Reveal className="mb-6 inline-flex items-center gap-2 text-sm text-muted-foreground">
          ...
          <ChevronRight className="h-4 w-4" />
          <span className="text-foreground">Chimney</span>
        </Reveal>

        <div className="grid gap-8 lg:grid-cols-[1.45fr_1fr] lg:items-end">
          <Reveal className="max-w-3xl">
            <div className="mb-6 inline-flex items-center gap-2 rounded-full border border-secondary/30 bg-black/20 px-4 py-1.5"><span className="text-sm font-medium text-secondary">Professional Service</span></div>
            <h1 className="mb-6 font-serif text-4xl font-bold text-foreground md:text-5xl lg:text-6xl">Chimney Services</h1>
            <p className="mb-8 max-w-2xl text-lg leading-relaxed text-muted-foreground md:text-xl">Complete chimney repair, rebuilding, flashing, and waterproofing services.</p>
            <div className="flex flex-col gap-4 sm:flex-row">
              <Button asChild size="lg" className="h-12 px-6 text-base font-semibold"><Link href="/contact">Get Free Estimate<ArrowRight className="ml-1 h-5 w-5" /></Link></Button>
              <Button asChild size="lg" variant="outline" className="h-12 border-secondary/40 px-6 text-base text-secondary hover:bg-secondary/10 hover:text-secondary"><a href={`tel:${siteConfig.phones.english}`}><Phone className="mr-2 h-5 w-5" />{siteConfig.phones.english}</a></Button>
            </div>
          </Reveal>
